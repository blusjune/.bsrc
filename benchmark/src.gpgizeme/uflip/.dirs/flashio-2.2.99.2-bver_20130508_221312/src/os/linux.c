            ...
            int          flags)
{
  ...
  if (buf != 0)
...
#if 1 /* blusjune:: to handle properly
	 '/dev/sdb1'	--> /sys/block/sdb/queue/scheduler,
	 '/dev/md0'	--> /sys/block/md0/queue/scheduler,
	 '/dev/md_d0'	--> /sys/block/md_d0/queue/scheduler,
	 as well as
	 '/dev/sdb'	--> /sys/block/sdb/queue/scheduler
	 */
	...
	do {
		...
		for (i=0; device->data->str[i] != 0; i++) {
			dev_id_str[i] = device->data->str[i];
		} /* now we get the string length in variable 'i' */
		dev_id_str[i] = 0;

		if (dev_id_str[5] == 'm' && dev_id_str[6] == 'd') {
			break;	
		}
		else {
			i--;
			for (; i>=0; i--) {
				if (dev_id_str[i] >= 0x30 && dev_id_str[i] <= 0x39) {
					dev_id_str[i] = 0;
				}
				else {
					break;
	...
	printf("device->data->str: (%s)\n", device->data->str);
	printf("dev_id_str: (%s)\n", dev_id_str);

	sprintf (path, "%s/%s/%s", IO_SCHED_PATH_PREFIX, dev_id_str + 5, IO_SCHED_PATH_SUFFIX);
#else
      sprintf (path, "%s/%s/%s", IO_SCHED_PATH_PREFIX, device->data->str + 5, IO_SCHED_PATH_SUFFIX);
#endif
#if 1
	printf("IO_SCHED_PATH_PREFIX(%s)\n", path);
#endif
...
#if 0 /* blusjune:: original uFLIP code */

  ret = ioctl (fd, HDIO_GETGEO, &geom);

#else /* blusjune:: blusjune's modification
	 to get storage device geometry even in the RAID case */

  do {
	unsigned long dev_size_in_sectors = 0; /* storage device size in 512-byte sectors */
	unsigned long logical_sector_size = 0; /* logical sector(block) size in bytes */

	ret = ioctl(fd, BLKGETSIZE64, &dev_size_in_sectors);
	ret = ioctl(fd, BLKSSZGET, &logical_sector_size);

	dg [0] = dev_size_in_sectors / logical_sector_size;
	dg [1] = 1;
	dg [2] = 1;
	dg [3] = 512;

#if 0
	geom.cylinders = dev_size_in_sectors/logical_sector_size;
	geom.heads = 1;
	geom.sectors= 1;

	printf("dev_size_in_sectors : %lu (%x)\n", dev_size_in_sectors, dev_size_in_sectors);
	printf("logical_sector_size : %lu (%x)\n", logical_sector_size, logical_sector_size);
	printf("geom.cylinders      : %d (%x)\n", geom.cylinders, geom.cylinders);
	printf("dev_size_in_sectors / logical_sector_size (%%lu) : %lu\n",
			dev_size_in_sectors / logical_sector_size);
	printf("dev_size_in_sectors / logical_sector_size (%%d) : %d\n",
			dev_size_in_sectors / logical_sector_size);
#endif
